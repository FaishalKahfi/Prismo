...
class PrismoHistoryManager {
    ...
    initializeView() {
        // Set default date from input
        const filterDateInput = document.getElementById('filter-date');
        if (filterDateInput) {
            this.currentDate = filterDateInput.value || this.currentDate;
        }
        ...
        // DON'T load from API - use server-rendered data
        // Daily view is already active and data is already displayed
        console.log('Using server-rendered data, no API call needed');
    }
    ...
    showDailyView() {
        ...
        if (dailyView) {
            ...
            // Data already rendered by Blade, no need to re-render
        }
    }
    ...
    showMonthlyView() {
        ...
        if (monthlyView) {
            ...
            // Data already rendered by Blade
        }
    }
    ...
    filterByDate() {
        ...
        window.location.href = `/mitra/saldo/history?date=${selectedDate}`;
    }
    ...
    setupExportButtons() {
        ...
        if (exportPdfBtn) {
            exportPdfBtn.addEventListener('click', () => {
                window.location.href = `/mitra/saldo/export?type=${this.currentView}&format=pdf&date=${this.currentDate}`;
            });
        }
        
        if (exportExcelBtn) {
            exportExcelBtn.addEventListener('click', () => {
                window.location.href = `/mitra/saldo/export?type=${this.currentView}&format=csv&date=${this.currentDate}`;
            });
        }
    }
    ...
    exportPDF() {
        ...
        window.location.href = `/mitra/saldo/export?type=${this.currentView}&format=pdf&date=${filterDate}`;
    }

    exportExcel() {
        console.log('Exporting to Excel...');
        const filterDate = document.getElementById('filter-date')?.value || this.currentDate;
        window.location.href = `/mitra/saldo/export?type=${this.currentView}&format=csv&date=${filterDate}`;
    }
