...
let bookingData = {
    ...
    time: null,
    service: null,
    treatment: null, // Will be set from URL
    price: null,
    tipe: null,
    nopol: null,
    merek: null,
...
// Set treatment from URL parameter
bookingData.treatment = serviceType;

// Get mitra business data from backend or use defaults
const mitraData = window.mitraBusinessData || {};

// Get max slots for selected service (defined before BUSINESS_HOURS)
...
// Calculate max slots now (before using in BUSINESS_HOURS)
const maxSlotsForCurrentService = getMaxSlotsForService();
...
const BUSINESS_HOURS = {
    ...
    maxSlotsPerTime: maxSlotsForCurrentService, // Use calculated value
...
document.addEventListener("DOMContentLoaded", function () {
    console.log("🚀 DOM Content Loaded - Starting Booking System");

    try {
        console.log("📊 Checking mitraBusinessData:", window.mitraBusinessData);
        ...
        alert(
            "Terjadi error saat loading halaman booking. Silakan refresh halaman atau hubungi admin."
        );
    }
});
...
async function loadBookedSlots() {
    try {
        ...
        const response = await fetch(
            `/api/bookings/slots/${mitraId}?start_date=${
        ...
        window.SLOT_DETAILS = data.slot_details || {};
        window.SERVICE_SLOTS = data.services || {};
        console.log("✅ Booked slots loaded from API:", BOOKED_SLOTS);
        console.log("📊 SLOT_DETAILS:", window.SLOT_DETAILS);
        console.log("🎯 SERVICE_SLOTS:", window.SERVICE_SLOTS);
        console.log("🔍 Selected service:", bookingData.treatment);
        console.log("📊 SLOT_DETAILS:", window.SLOT_DETAILS);
        console.log("🎯 SERVICE_SLOTS:", window.SERVICE_SLOTS);
        console.log("🔍 Selected service:", bookingData.treatment);
        console.log("✅ Slot details:", window.SLOT_DETAILS);
        console.log("✅ Service slots:", window.SERVICE_SLOTS);
...
function generateTimeSlots() {
    ...
    while (currentMinutes < endMinutes) {
        ...
        // Check if this slot is fully booked
            ...
            console.log(`🔴 Slot ${timeString} marked as BOOKED (red)`);
        }
        // Available slot
        else {
            const selectedService = bookingData.treatment;
            const serviceSlots = window.SERVICE_SLOTS || {};
            const maxSlots = selectedService
                ? serviceSlots[selectedService]?.max_slots || 1
                : 1;
            ...
            timeSlot.title = selectedService
                ? `Tersedia ${availableSlots} slot untuk ${selectedService} - ${timeString}`
                : `${timeString}`;
            timeSlot.addEventListener("click", () =>
                selectTime(timeString, timeSlot)
            );
        }
...
function isSlotBooked(timeString) {
    // Check if this specific service at this time is full
    const selectedService = bookingData.treatment;
    if (!selectedService) {
        console.warn("⚠️ No service selected, cannot check slot availability");
        return false;
    }

    const selectedDate = formatDateForAPI(bookingData.date);
    const slotDetails = window.SLOT_DETAILS || {};
    const serviceSlots = window.SERVICE_SLOTS || {};

    // Get max slots for selected service
    const maxSlots = serviceSlots[selectedService]?.max_slots || 1;

    // Get used slots for this service at this time
    const usedSlots =
        slotDetails[selectedDate]?.[timeString]?.[selectedService] || 0;

    const isBooked = usedSlots >= maxSlots;

    // Debug logging
    console.log(`🔍 Checking ${timeString} on ${selectedDate}:`);
    console.log(`  - Service: ${selectedService}`);
    console.log(`  - Max slots: ${maxSlots}`);
    console.log(`  - Used slots: ${usedSlots}`);
    console.log(`  - Is booked: ${isBooked}`);
    console.log(`  - Slot details for this date:`, slotDetails[selectedDate]);

    return isBooked;
}

function getBookedCount(timeString) {
    // Return used slots for selected service at this time
    const selectedService = bookingData.treatment;
    ...
    return slotDetails[selectedDate]?.[timeString]?.[selectedService] || 0;
}
...
function updateBusinessHoursDisplay(dateStr) {
    ...
    console.log(
        `🔍 updateBusinessHoursDisplay - dateStr: ${dateStr}, dayOfWeek: ${dayOfWeek}`
    );
    console.log(
        `🔍 BREAK_SCHEDULES[${dayOfWeek}]:`,
        BREAK_SCHEDULES[dayOfWeek]
    );
...
function selectTime(time, element) {
    ...
    if (element.classList.contains("booked")) {
        showAlert(
            "Slot Sudah Dipesan",
            "Maaf, slot ini sudah dipesan oleh customer lain. Silakan pilih slot lainnya."
        );
        return;
    }
    ...
    if (!isWithinBusinessHours(time)) {
        ...
        showAlert(
            ...
            `Maaf, jam ${time} berada di luar jam operasional (${hours.open} - ${hours.close}).`
        );
        return;
    }
...
function isBreakTime(time) {
    ...
    return breaks.some((breakPeriod) => {
        ...
        return (
            selectedTimeInMinutes >= breakStartInMinutes &&
            selectedTimeInMinutes < breakEndInMinutes
        );
    });
}
...
function getBreakReason(time) {
    ...
    return breakPeriod
        ? `Istirahat (${breakPeriod.start} - ${breakPeriod.end})`
        : "Waktu Istirahat";
}
...
function generateCalendar() {
    ...
    console.log("📅 Generating calendar for booking...");
    ...
    for (let day = 1; day <= daysInMonth; day++) {
        ...
        const isOperationalDay =
            BUSINESS_HOURS.operationalDays.includes(dayOfWeek);
        ...
        if (day < today || !isOperationalDay || !isMitraOpen) {
            ...
            dayElement.addEventListener("click", () =>
                selectDate(day, month, year, dayElement)
            );
        }
...
function selectDate(day, month, year, element) {
    ...
    console.log(
        `🔍 DEBUG - formatDateForAPI result: ${formatDateForAPI(dateStr)}`
    );
    console.log(
        `🔍 DEBUG - BOOKED_SLOTS for this date:`,
        BOOKED_SLOTS[formatDateForAPI(dateStr)]
    );
    console.log(
        `🔍 DEBUG - BREAK_SCHEDULES for this date:`,
        getBusinessHours(dateObj).breaks
    );
...
function initializeBookingSystem() {
    ...
    if (calendarGrid) {
        ...
        console.error("❌ Calendar grid not found in DOM!");
    }
...
function calculateTotal() {
    ...
    // Get voucher discount first (only applies to service price, not admin fee)
    const voucherDiscount = appliedVoucher ? appliedVoucher.discount : 0;

    // Calculate subtotal after voucher but before points (servicePrice - voucher + adminFee)
    const subtotalAfterVoucher = servicePrice - voucherDiscount + adminFee;

    // Get user's available points from backend
    const availablePoints = window.mitraBusinessData?.customerPoints || 0;

    // Each point = Rp 1 (customer gets 10 points per completed booking)
    const pointValue = 1;

    // Calculate points to use: can use all available points up to subtotal
    // If subtotal is 2002, we can use up to 2002 points (Rp 2002)
    // If subtotal is 1500, we can use up to 1500 points (Rp 1500)
    const pointsToUse = usePoints
        ? Math.min(Math.floor(subtotalAfterVoucher), availablePoints)
        : 0;
    ...
    if (pointSection && pointDiscountEl) {
        ...
        pointDiscountEl.textContent = `-Rp ${pointDiscount.toLocaleString(
            "id-ID"
        )} (${pointsToUse} Point)`;
    }
    ...
    // Calculate total (servicePrice - voucher + adminFee - points)
    const total = Math.max(
        0,
        servicePrice - voucherDiscount + adminFee - pointDiscount
    );
...
function handleFile(file) {
    ...
    reader.onload = function (e) {
        ...
        console.log(
            `📤 File uploaded: ${file.name} (${(file.size / 1024).toFixed(
                2
            )}KB)`
        );
    };
...
async function submitPayment() {
    ...
    try {
        ...
        const bookingPayload = {
            mitra_id:
                mitraData.mitraId || window.location.pathname.split("/").pop(),
            service_type: bookingData.service || serviceType,
            vehicle_type: bookingData.tipe || "Avanza",
            ...
            booking_time:
                bookingData.time && bookingData.time !== "-"
                    ? bookingData.time
                    : "09:00",
            base_price: bookingData.basePrice || servicePrice,
            discount_amount:
                bookingData.voucherDiscount + bookingData.pointsUsed * 1,
        ...
        const uploadResponse = await fetch(
            `/api/bookings/${bookingResult.id}/payment-proof`,
            {
                method: "POST",
                headers: {
                    Accept: "application/json",
        ...
        if (successMessage) {
            ...
            const successSubtext =
                successMessage.querySelector(".success-subtext");
            if (successSubtext) {
        ...
        const invoiceDetails = {
            partnerName:
                mitraData.businessName ||
                (partnerNameEl ? partnerNameEl.textContent : "Mitra"),
            partnerLocation: `${mitraData.city || "Jakarta"}, ${
                mitraData.province || "DKI Jakarta"
            }`,
            partnerAddress: mitraData.address || "Alamat tidak tersedia",
            partnerPhone:
                mitraData.phone || (phoneEl ? phoneEl.textContent : "-"),
...
function updateInvoiceUI() {
    ...
    document.getElementById("partnerAddress").textContent =
        invoiceData.partnerAddress;
    document.getElementById("partnerPhone").textContent =
...
// ========== VOUCHER MANAGEMENT ==========
// Get claimed vouchers from backend
const AVAILABLE_VOUCHERS = {};

// Initialize vouchers from business data
if (window.mitraBusinessData && window.mitraBusinessData.claimedVouchers) {
    window.mitraBusinessData.claimedVouchers.forEach((voucher) => {
        // If has percentage, use it; otherwise use fixed/maxDiscount
        const discountValue =
            voucher.discountPercent ||
            voucher.maxDiscount ||
            voucher.discountFixed;

        AVAILABLE_VOUCHERS[voucher.code] = {
            code: voucher.code,
            discount: discountValue,
            type: voucher.type,
            description: voucher.description,
            minPurchase: voucher.minTransaction || 0,
...
console.log("✅ Loaded vouchers:", AVAILABLE_VOUCHERS);
...
function applyVoucher() {
    ...
    if (voucher.minPurchase && servicePrice < voucher.minPurchase) {
        const minAmount = Math.floor(voucher.minPurchase).toLocaleString(
            "id-ID"
        );
        showVoucherMessage(
            `Minimum pembelian Rp ${minAmount} untuk menggunakan voucher ini`,
            "error"
        );
        appliedVoucher = null;
        updateSummary();
    ...
    if (voucher.discountPercent) {
        ...
        // Fixed discount (using maxDiscount or discount value)
    ...
    // Update UI
    showVoucherMessage(
        `Voucher berhasil diterapkan! ${voucher.description}`,
        "success"
    );
