...
console.log("Approval Mitra (pending):", mitrasData.approvalMitra);
console.log("Active Mitra (approved):", mitrasData.activeMitra);
console.log("Total pending:", mitrasData.approvalMitra.length);
console.log("Total approved:", mitrasData.activeMitra.length);
...
class ModalManager {
    ...
    populateModal(modalName, mitra) {
        switch (modalName) {
            case "ban":
                document.getElementById("banMitraName").textContent =
                    mitra.namaTempat;
                break;
            case "unban":
                document.getElementById("unbanMitraName").textContent =
                    mitra.namaTempat;
                break;
        }
    }
    ...
    handleBan() {
        ...
        setTimeout(() => {
            ...
            showNotification(
                `Mitra "${mitra.namaTempat}" berhasil dinonaktifkan`,
                "success"
            );
    ...
    handleUnban() {
        ...
        setTimeout(() => {
            ...
            showNotification(
                `Mitra "${mitra.namaTempat}" berhasil diaktifkan kembali`,
                "success"
            );
...
// ===== DATA MANAGEMENT =====
function loadMitrasData() {
    console.log("=== loadMitrasData called ===");
    renderApprovalData();
    renderTableData();
}
...
function renderApprovalData() {
    ...
    // Update count
    if (countElement) {
        countElement.textContent = `(${mitrasData.approvalMitra.length})`;
    }

    // Clear container
    container.innerHTML = "";

    if (mitrasData.approvalMitra.length === 0) {
        container.innerHTML = `
    ...
    mitrasData.approvalMitra.forEach((mitra, index) => {
        ...
        row.innerHTML = `
            ...
            <div class="approval-col approval-action">
                <a href="/admin/kelolamitra/${
                    mitra.id
                }/form" class="btn btn-primary btn-sm">Detail</a>
            </div>
        `;
...
function renderTableData() {
    ...
    console.log("mitrasData.activeMitra:", mitrasData.activeMitra);
    console.log("activeMitra length:", mitrasData.activeMitra.length);

    // Calculate pagination
    const startIndex = (currentPage - 1) * itemsPerPage;
    const endIndex = startIndex + itemsPerPage;
    const paginatedData = mitrasData.activeMitra.slice(startIndex, endIndex);
            ...
            <tr>
                <td colspan="9" class="empty-state">
                    <div>
                        ...
                        <span class="empty-subtitle">Semua mitra telah diproses</span>
                    </div>
                </td>
            </tr>
        `;
    ...
    paginatedData.forEach((mitra, index) => {
        ...
        const formattedRating =
            rating % 1 === 0 ? rating.toFixed(1) : rating.toFixed(1);
        ...
        row.innerHTML = `
            ...
            <td>
                <button class="btn btn-info btn-sm" onclick="viewMitraDetail(${
                    mitra.id
                })" style="margin-right: 8px;">
                    Detail
                </button>
                <button class="btn ${
                    mitra.status === "Nonaktif" ? "btn-success" : "btn-danger"
                } btn-sm ban-btn" data-id="${mitra.id}">
...
function renderPagination() {
    const totalPages = Math.ceil(mitrasData.activeMitra.length / itemsPerPage);
...
function initTableActions() {
    ...
    tableBody.addEventListener("click", async function (e) {
        ...
        const mitra = mitrasData.activeMitra.find((m) => m.id == mitraId);
        ...
        // Konfirmasi action
        const action =
            mitra.status === "Nonaktif" ? "mengaktifkan" : "menonaktifkan";
        const confirmation = confirm(
            `Apakah Anda yakin ingin ${action} mitra ${mitra.pemilik}?`
        );
        ...
        try {
            ...
            console.log(
                "Sending request to:",
                `/admin/user/${mitraId}/toggle-status`
            );
            ...
            if (result.success) {
                // Update mitra status di mitrasData
                ...
                throw new Error(
                    result.message || "Gagal mengubah status mitra"
                );
            }
        } catch (error) {
            console.error("Error toggling mitra status:", error);
            showNotification(
                error.message || "Terjadi kesalahan saat mengubah status mitra",
                "error"
            );
    ...
    if (existingNotification) {
        ...
        setTimeout(() => {
            if (existingNotification.parentNode) {
                existingNotification.parentNode.removeChild(
...
window.MitraManager = {
    ...
    addMockApproval: function () {
        const newMitra = {
            id:
                "AP" +
                (mitrasData.approvalMitra.length + 1)
                    .toString()
                    .padStart(3, "0"),
            namaTempat: "New Car Wash " + (mitrasData.approvalMitra.length + 1),
        ...
        mitrasData.approvalMitra.push(newMitra);
        renderApprovalData();
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    console.log("Mitras Data ready:", mitrasData);
});
