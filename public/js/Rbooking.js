...
function loadUserProfile() {
    // Avatar images and user names are loaded from database via Blade template
    // No additional JavaScript processing needed as names are already rendered server-side
    console.log("✅ Booking page: User profile loaded from server");
}
...
function setStarRating(rating) {
    ...
    if (ratingText) {
        ratingText.textContent =
            RATING_DESCRIPTIONS[rating] || `Rating: ${rating} bintang`;
    }
}
    ...
    Array.from(files).forEach((file) => {
        ...
        reader.onload = function (e) {
            ...
            previewDiv.innerHTML = `
                ...
                <button class="remove-preview" onclick="removePhoto(${
                    currentReviewData.photos.length - 1
                })">
...
function removePhoto(index) {
    ...
    console.log(
        "🗑️ Photo removed, remaining:",
        currentReviewData.photos.length
    );
}
...
function openReviewModal(booking, isEditing = false) {
    ...
    const currentReviewSection = document.getElementById(
    ...
    if (currentReviewSection) {
        if (isEditing && booking.comment) {
            ...
            document.getElementById("currentRatingDisplay").innerHTML =
                generateStars(booking.rating);
            document.getElementById("currentCommentDisplay").textContent =
                booking.comment;
        } else {
    ...
    if (modal) {
        ...
        console.log(
            `⭐ Review modal opened for ${booking.partner} (${
...
function submitReview() {
    ...
    const bookingIndex = bookingHistory.findIndex(
        (b) => b.id === currentReviewData.bookingId
    );
    if (bookingIndex === -1) {
    ...
    // Use bookingCode if available, fallback to id for backward compatibility
    const booking = bookingHistory.find(
        (b) => b.id === currentReviewData.bookingId
    );
    const bookingCode = booking?.bookingCode || currentReviewData.bookingId;
    ...
    // Send to backend
    const endpoint = currentReviewData.isEditing
    ...
    fetch(endpoint, {
        ...
        .then((data) => {
            if (data.success) {
                ...
                bookingHistory[bookingIndex].comment =
                ...
                const photoText =
                    currentReviewData.photos.length > 0
...
function initializeNavigation() {
    ...
    navLinks.forEach((link) => {
        ...
        if (
            currentPage === href ||
            (currentPage.includes("/customer/booking") &&
...
function initializeMobileMenu() {
    ...
    if (!menuToggle || !mainNav) {
        console.log(
            "Mobile menu elements not found, skipping mobile menu initialization"
        );
        return;
    }
...
function loadBookingData() {
    ...
    console.log("📦 Booking data loaded from server", {
        currentBooking,
    ...
    if (bookingCurrent) {
        ...
        if (!bookingCurrent.querySelector(".empty-booking-state")) {
            ...
            emptyState.style.cssText =
                "text-align: center; padding: 60px 20px; color: #666; background: #f8f9fa; border-radius: 12px; margin-top: 20px;";
...
function updateCurrentBookingDisplay() {
    ...
    document.getElementById("currentPartner").textContent =
        currentBooking.partner;
    document.getElementById(
        "currentPrice"
    ).textContent = `Total: ${formatCurrency(currentBooking.totalPrice)}`;
    ...
    document.getElementById("currentTreatment").textContent =
        currentBooking.treatment;
...
function handleBookAgain(booking) {
    ...
    console.log("🔄 Booking again with data:", booking);

    // Validate mitraId exists
    if (!booking.mitraId) {
        console.error("❌ No mitra ID found in booking data:", booking);
        alert(
            "Tidak dapat membuat booking ulang. Data mitra tidak ditemukan. Silakan refresh halaman."
        );
        return;
    }
    ...
    console.log(
        "✅ Navigating to:",
        `/customer/atur-booking/booking/${booking.mitraId}?${params.toString()}`
    );

    // Navigate to booking page with mitra ID and parameters
...
function initializeCancelModal() {
    // No form handling needed - refund method is set in profile
...
function openCancelModal() {
    ...
    if (statusValue >= BOOKING_STATUS.IN_PROGRESS) {
        alert(
            "Booking tidak dapat dibatalkan karena sudah dalam proses atau selesai"
        );
        return;
    }
...
function confirmCancelBooking() {
    ...
    fetch(`/api/bookings/${currentBooking.id}/cancel`, {
        method: "POST",
        headers: {
        ...
        .then((response) => {
            if (!response.ok) {
                return response.json().then((err) => {
                    throw new Error(err.message || "Gagal membatalkan booking");
                });
            }
        ...
        .then((data) => {
            ...
            const successModalContent =
                successModal.querySelector(".success-modal");
            ...
            if (successModalContent) {
                ...
                <p style="margin: 15px 0; color: #666; font-size: 14px;">
                    Dana Rp ${formatCurrency(
                        currentBooking.totalPrice - 2500
                    ).replace(
                        "Rp ",
                        ""
                    )} akan dikembalikan ke metode refund yang Anda atur di profil.<br><br>
        ...
        .catch((error) => {
            ...
            if (cancelBtn) {
                cancelBtn.disabled = false;
                cancelBtn.textContent = "Batalkan Booking";
            }
        });
}
...
function initializeRescheduleModal() {
    // Don't generate calendar here - will be generated after mitra data is loaded
    console.log(
        "📅 Reschedule modal system initialized (calendar will be generated when modal opens)"
    );
}
...
function generateRescheduleCalendar() {
    ...
    if (!calendarContainer) {
        console.error("❌ Reschedule calendar container not found!");
        return;
    }
    ...
    header.innerHTML = `<h3>${months[month]} ${year}</h3>`;
    ...
    // Get operational days from mitra data
    let operationalDays = []; // Start with empty array

    console.log("🔍 Checking mitraBusinessData for operational hours...");
    console.log("mitraBusinessData:", mitraBusinessData);

    // Check if API already provided parsed operationalDays array (preferred)
    if (mitraBusinessData && Array.isArray(mitraBusinessData.operationalDays)) {
        operationalDays = mitraBusinessData.operationalDays;
        console.log(
            "✅ Using pre-parsed operationalDays from API:",
            operationalDays
        );
    }
    // Otherwise, parse operational_hours manually
    else {
        console.log(
            "⚠️ operationalDays not found in API, parsing operational_hours manually..."
        );

        const mitraProfile =
            mitraBusinessData?.mitraProfile || mitraBusinessData?.mitra_profile;

        console.log("🔍 mitraProfile found:", mitraProfile);

        if (mitraProfile && mitraProfile.operational_hours) {
            const operationalHours = mitraProfile.operational_hours;
            console.log(
                "📋 Operational hours from database:",
                operationalHours
            );
            console.log(
                "📋 Type of operational_hours:",
                typeof operationalHours
            );
            console.log(
                "📋 Keys in operational_hours:",
                Object.keys(operationalHours)
            );

            // Convert operational_hours object to array of enabled day numbers
            const dayNames = [
                "sunday",
                "monday",
                "tuesday",
            ...
            dayNames.forEach((dayName, dayIndex) => {
                const dayConfig = operationalHours[dayName];
                console.log(`  Day ${dayName} (${dayIndex}):`, dayConfig);
                if (dayConfig && dayConfig.enabled === true) {
                    console.log(`    ✅ ${dayName} is operational`);
                    operationalDays.push(dayIndex);
                } else {
            ...
            console.log(
                "✅ Manually parsed operational days:",
                operationalDays
            );
        } else {
            // Fallback: Default Monday-Friday if no data
            console.warn(
                "⚠️ No operational hours found, using default Monday-Friday"
            );
            console.warn("mitraBusinessData:", mitraBusinessData);
            console.warn("mitraProfile:", mitraProfile);
            if (mitraProfile) {
                console.warn(
                    "Keys in mitraProfile:",
                    Object.keys(mitraProfile)
                );
            }
    ...
    console.log("🎯 Final operational days to use:", operationalDays);

    // Add day cells for current month only
    for (let day = 1; day <= daysInMonth; day++) {
        ...
        const isOperationalDay = operationalDays.includes(dayOfWeek);
        ...
        if (day <= 7) {
            ...
            console.log(
                `Day ${day} (${dayNames[dayOfWeek]}): isOperational=${isOperationalDay}, dayOfWeek=${dayOfWeek}`
            );
        }
        ...
        // Disable past dates and non-operational days
        if (day < today || !isOperationalDay) {
            dayElement.classList.add("disabled");
            if (!isOperationalDay) {
                dayElement.classList.add("closed"); // Red color for closed days
                if (day <= 7) {
                    console.log(`  -> Day ${day} marked as CLOSED (red)`);
                }
            }
        } else {
            dayElement.addEventListener("click", () =>
                selectRescheduleDate(day, month, year, dayElement)
            );
        }
    ...
    console.log(
        `📅 Reschedule calendar generated: ${daysInMonth} days, current month only`
    );
}

function selectRescheduleDate(day, month, year, element) {
    // Remove previous selection
    const allDays = document.querySelectorAll(
        "#rescheduleCalendar .calendar-day"
    );
    ...
    rescheduleData.date = formattedDate;

    console.log(`📅 Reschedule date selected: ${formattedDate}`);
...
async function loadTimeSlots(selectedDate) {
    ...
    console.log(
        "🔄 Loading time slots for:",
        selectedDate,
        "Mitra ID:",
        currentBooking.mitraId
    );
    ...
    if (!mitraBusinessData) {
        try {
            console.log(
                "📥 Fetching mitra data from:",
                `/api/mitra/${currentBooking.mitraId}`
            );
            ...
            if (response.ok) {
                ...
                console.log(
                    "🔍 Has mitraProfile?",
                    !!mitraBusinessData.mitraProfile
                );
                console.log(
                    "🔍 Has mitra_profile?",
                    !!mitraBusinessData.mitra_profile
                );
                ...
                if (profile) {
                    ...
                    console.error("❌ No profile found in API response");
                }
            } else {
                console.error("❌ Failed to load mitra data:", response.status);
                if (timeSelect) {
                    timeSelect.innerHTML =
                        '<option value="">Gagal memuat data mitra</option>';
                }
                return;
            }
        } catch (error) {
    ...
    if (!bookedSlotsData[selectedDate]) {
        try {
            ...
            const response = await fetch(
                `/api/bookings/slots/${currentBooking.mitraId}?start_date=${startDate}&end_date=${endDate}`
            );
            if (response.ok) {
                const data = await response.json();
...
function generateRescheduleTimeSlots(selectedDate) {
    ...
    if (!mitraBusinessData) {
        ...
        timeSelect.innerHTML =
            '<option value="">Gagal memuat data mitra. Silakan refresh halaman.</option>';
        return;
    }

    console.log(
        "🔍 Full mitraBusinessData:",
        JSON.stringify(mitraBusinessData, null, 2)
    );

    // Check both camelCase and snake_case for mitra_profile
    const mitraProfile =
        mitraBusinessData.mitraProfile ||
        mitraBusinessData.mitra_profile ||
        mitraBusinessData.mitra_profiles; // some APIs return plural

    if (!mitraProfile) {
        console.error("❌ Mitra profile not found in data");
        console.error("Available keys:", Object.keys(mitraBusinessData));
        timeSelect.innerHTML =
            '<option value="">Data profil mitra tidak ditemukan. Silakan hubungi admin.</option>';
        return;
    }

    console.log("✅ Mitra profile found:", mitraProfile);
    ...
    if (!dailyHours || Object.keys(dailyHours).length === 0) {
        console.error("❌ No operational_hours found");
        console.error("mitraProfile keys:", Object.keys(mitraProfile));
        timeSelect.innerHTML =
            '<option value="">Jam operasional belum diatur. Silakan hubungi mitra.</option>';
        return;
    }
    ...
    if (!hours || !hours.enabled) {
        timeSelect.innerHTML =
            '<option value="">Mitra tutup pada hari ini</option>';
    ...
    if (!hours.open || !hours.close) {
        ...
        timeSelect.innerHTML =
            '<option value="">Format jam operasional tidak valid</option>';
        return;
    }
    ...
    if (
        mitraProfile &&
        mitraProfile.custom_services &&
        Array.isArray(mitraProfile.custom_services)
    ) {
        ...
        for (let service of mitraProfile.custom_services) {
            ...
            if (
                service.name === currentBooking.treatment ||
                service.service_name === currentBooking.treatment
            ) {
    ...
    while (currentMinutes < endMinutes) {
        ...
        const isDuringBreak = breaks.some((breakPeriod) => {
            const [breakStartHour, breakStartMinute] = breakPeriod.open
                .split(":")
                .map(Number);
            const [breakEndHour, breakEndMinute] = breakPeriod.close
                .split(":")
                .map(Number);
        ...
        const bookedCount =
            bookedSlotsData[selectedDate]?.filter((t) => t === timeString)
                .length || 0;
...
async function openRescheduleModal() {
    ...
    if (statusValue >= BOOKING_STATUS.IN_PROGRESS) {
        alert(
            "Booking tidak dapat direschedule karena sudah dalam proses atau selesai"
        );
        return;
    }
    ...
    if (!mitraBusinessData) {
        try {
            ...
            if (response.ok) {
                ...
                console.log(
                    "🔍 Has operationalDays?",
                    !!mitraBusinessData.operationalDays
                );
                console.log(
                    "🔍 operationalDays value:",
                    mitraBusinessData.operationalDays
                );
            } else {
                console.error(
                    "❌ Failed to preload mitra data:",
                    response.status
                );
    ...
    // Generate calendar AFTER mitra data is loaded
    console.log("📅 Generating calendar with loaded mitra data...");
    ...
    if (modal) {
        ...
        document.body.style.overflow = "hidden";
        console.log(
            "📅 Reschedule modal opened for mitra:",
            currentBooking.mitraId
        );
    }
}
...
function confirmReschedule() {
    ...
    if (!currentBooking || !currentBooking.bookingCode) {
        alert("Data booking tidak ditemukan!");
        console.error("❌ currentBooking:", currentBooking);
        return;
    }

    console.log("✅ Reschedule confirmed:", rescheduleData);
    console.log(
        "📤 Sending reschedule request for booking:",
    ...
    // Send reschedule request to backend with CSRF token
    fetch("/customer/booking/reschedule", {
        ...
        credentials: "include",
        body: JSON.stringify({
            booking_code: String(currentBooking.bookingCode), // Use bookingCode, not id
            new_date: String(rescheduleData.date),
            new_time: String(rescheduleData.time),
        }),
    })
        .then(async (response) => {
            ...
            try {
                ...
                if (data.success) {
                    ...
                    if (currentBooking) {
                        currentBooking.date = formatDateToIndonesian(
                            rescheduleData.date
                        );
                ...
                if (
                    ...
                    alert(
                        "Sesi Anda telah berakhir. Silakan refresh halaman dan login kembali."
                    );
                } else if (response.status === 419) {
                    alert(
                        "Halaman sudah terlalu lama dibuka. Silakan refresh halaman."
                    );
                } else {
                    alert(
                        "Terjadi kesalahan pada server. Status: " +
                            response.status
                    );
                }
            }
...
window.confirmCancel = confirmCancelBooking; // Fixed: match the actual function name
