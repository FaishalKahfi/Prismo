...
class PrismoDashboardManager {
    constructor() {
        ...
        this.storeStatus =
            typeof window.mitraIsOpen !== "undefined" && window.mitraIsOpen
                ? "open"
                : "closed";
    ...
    getInitialServices() {
        ...
        console.log(
            "window.initialCustomServices:",
            window.initialCustomServices
        );
        console.log(
            "window.initialServicePackages:",
            window.initialServicePackages
        );
        ...
        if (
            window.initialCustomServices &&
            window.initialCustomServices.length > 0
        ) {
            console.log(
        ...
        if (
            window.initialServicePackages &&
            window.initialServicePackages.length > 0
        ) {
            console.log(
                "✅ Loading from service packages:",
                window.initialServicePackages
            );
        ...
        return new Promise((resolve, reject) => {
            ...
            fetch("/mitra/update-custom-services", {
                ...
                .then((data) => {
                    if (data.success) {
                        console.log(
                            "Custom services saved successfully:",
                            data.services
                        );
                        resolve(data);
                    } else {
                        reject(
                            new Error(
                                data.message ||
                                    "Gagal menyimpan custom services"
                            )
                        );
                    }
                })
    ...
    getInitialOperationalHours() {
        // Debug log
        console.log("🕐 Loading operational hours...");
        console.log(
            "window.initialOperationalHours:",
            window.initialOperationalHours
        );
        ...
        if (
            window.initialOperationalHours &&
            Object.keys(window.initialOperationalHours).length > 0
        ) {
            console.log("✅ Loaded operational hours from database");
            return window.initialOperationalHours;
        }

        // Default fallback - empty (tidak ada jam operasional)
        console.log("⚠️ No operational hours found - using empty defaults");
        return {
            monday: {
                enabled: false,
                open: "",
    ...
    init() {
        ...
        try {
            ...
            console.error(
                "❌ Failed to initialize PRISMO Dashboard Manager:",
                error
            );
        }
    }
    ...
    updateDesktopBadges() {
        ...
        if (antrianBadge) {
            ...
            if (count > 0) {
                ...
                antrianBadge.setAttribute(
                    "aria-label",
                    `${count} notifikasi antrian`
                );
            } else {
        ...
        if (reviewBadge) {
            ...
            if (count > 0) {
                ...
                reviewBadge.setAttribute(
                    "aria-label",
                    `${count} notifikasi review`
                );
            } else {
    ...
    updateMobileBadges() {
        ...
        const mobileAntrianBadge = document.getElementById(
            "mobile-antrian-badge"
        );
        ...
        const mobileReviewBadge = document.getElementById(
            "mobile-review-badge"
        );
    ...
    decrementNotification(type, amount = 1) {
        if (this.notifications.hasOwnProperty(type)) {
            this.notifications[type] = Math.max(
                0,
                this.notifications[type] - amount
            );
            this.updateAllBadges();
    ...
    setupFloatingWhatsApp() {
        ...
        floatingElement.addEventListener("touchstart", startDrag, {
            passive: false,
        });
    ...
    resetFloatingButtonPosition() {
        ...
        console.log(
            "📱 Floating button position reset for current screen size"
        );
    }
    ...
    setDefaultPosition(element) {
        ...
        console.log(
            "📍 Set default position for screen width:",
            window.innerWidth
        );
    }
        ...
        fetch("/mitra/update-status", {
            ...
            .then((data) => {
                if (data.success) {
                    this.updateStatusDisplay();
                    const statusText =
                        this.storeStatus === "open" ? "Buka" : "Tutup";
                    this.showAlert(
                        "success",
                        "Berhasil!",
                        `Status steam berhasil diubah menjadi ${statusText}`
                    );
                    console.log(
                        `🔄 Store status changed to: ${this.storeStatus}`
                    );
                } else {
                    this.showAlert(
                        "error",
    ...
    handleTimeInputChange(inputElement) {
        ...
        if (!timeRegex.test(timeValue)) {
            this.showTimeValidationError(
                inputElement,
                "Format waktu harus HH:MM (contoh: 08:00, 14:30)"
            );
            return;
        }
    ...
    validateTimePair(changedInput) {
        ...
        if (openMinutes >= closeMinutes) {
            ...
            this.showTimeValidationError(
                closeInput,
                "Jam tutup harus lebih akhir dari jam buka"
            );
        } else {
    ...
    validateBreakSchedules(day) {
        ...
        schedule.breakSchedules.forEach((breakSchedule, index) => {
            const breakOpenInput = document.getElementById(
                `break-open-${day}-${index}`
            );
            const breakCloseInput = document.getElementById(
                `break-close-${day}-${index}`
            );
            ...
            if (
                breakOpenMinutes < openMinutes ||
                breakCloseMinutes > closeMinutes
            ) {
                this.showTimeValidationError(
                    breakOpenInput,
                    "Jam istirahat harus dalam jam operasional"
                );
                this.showTimeValidationError(
                    breakCloseInput,
                    "Jam istirahat harus dalam jam operasional"
            ...
            if (breakOpenMinutes >= breakCloseMinutes) {
                this.showTimeValidationError(
                    breakOpenInput,
                    "Jam mulai istirahat harus lebih awal dari jam selesai"
                );
                this.showTimeValidationError(
                    breakCloseInput,
                    "Jam selesai istirahat harus lebih akhir dari jam mulai"
                );
                hasError = true;
            }
    ...
    showTimeValidationError(inputElement, message) {
        ...
        if (
            !validationElement ||
            !validationElement.classList.contains("time-validation-message")
        ) {
    ...
    displayOperationalSchedule() {
        ...
        days.forEach((day) => {
            ...
            if (schedule.enabled) {
                const scheduleElement = this.createScheduleElement(
                    day,
                    schedule
                );
    ...
    createScheduleElement(day, schedule) {
        ...
        element.innerHTML = `
            <div class="schedule-header">
                ...
                <div class="break-toggle-container">
                    <button type="button" class="break-toggle ${
                        schedule.hasBreakSchedule ? "break-toggle--active" : ""
                    }"
                            data-day="${day.key}">
                        ${
                            schedule.hasBreakSchedule
                                ? "Hapus Sesi Istirahat"
                                : "Atur Istirahat"
                        }
                    </button>
                    <button type="button" class="break-info-btn" data-day="${
                        day.key
                    }"
            ...
            ${
                schedule.hasBreakSchedule
                    ? this.createBreakSchedules(day.key, schedule)
                    : ""
            }
        `;
        ...
        if (openInput && closeInput) {
            openInput.addEventListener("input", () =>
                this.handleTimeInputChange(openInput)
            );
            openInput.addEventListener("blur", () =>
                this.handleTimeInputChange(openInput)
            );
            closeInput.addEventListener("input", () =>
                this.handleTimeInputChange(closeInput)
            );
            closeInput.addEventListener("blur", () =>
                this.handleTimeInputChange(closeInput)
            );
        }
    ...
    createBreakSchedules(dayKey, schedule) {
            ...
            breakSchedulesHTML += `
                <div class="break-schedule-item">
                    <div class="break-schedule-header">
                        <span class="break-session-label">Sesi Istirahat ${
                            index + 1
                        }</span>
                        ${
        ...
        breakSchedules.forEach((_, index) => {
            const breakOpenInput = breakSchedulesElement.querySelector(
                `#break-open-${dayKey}-${index}`
            );
            const breakCloseInput = breakSchedulesElement.querySelector(
                `#break-close-${dayKey}-${index}`
            );

            if (breakOpenInput && breakCloseInput) {
                breakOpenInput.addEventListener("input", () =>
                    this.handleTimeInputChange(breakOpenInput)
                );
                breakOpenInput.addEventListener("blur", () =>
                    this.handleTimeInputChange(breakOpenInput)
                );
                breakCloseInput.addEventListener("input", () =>
                    this.handleTimeInputChange(breakCloseInput)
                );
                breakCloseInput.addEventListener("blur", () =>
                    this.handleTimeInputChange(breakCloseInput)
                );
            }
        });
    ...
    toggleBreakSchedule(day, enabled = null) {
        ...
        if (schedule.hasBreakSchedule && schedule.breakSchedules.length === 0) {
            schedule.breakSchedules = [{ open: "12:00", close: "13:00" }];
        }
    ...
    showSaveConfirmation() {
        ...
        messageElement.textContent =
            "Apakah Anda yakin ingin menyimpan perubahan jam operasional?";
    ...
    validateAllTimes() {
        ...
        days.forEach((day) => {
            ...
            if (schedule.enabled) {
                ...
                if (openInput && closeInput) {
                    ...
                    if (
                        openInput.classList.contains("error") ||
                        closeInput.classList.contains("error")
                    ) {
                        isValid = false;
                    }
                }
        ...
        if (!isValid) {
            this.showAlert(
                "error",
                "Error",
                "Harap perbaiki kesalahan pada jam operasional sebelum menyimpan"
            );
        }
    ...
    saveOperationalHours() {
        ...
        days.forEach((day) => {
            ...
            if (schedule.enabled) {
                ...
                const openTime = openInput?.value || schedule.open;
                const closeTime = closeInput?.value || schedule.close;
                ...
                if (!openTime || !closeTime) {
                    errorMessage = `Harap masukkan jam operasional untuk ${this.getDayName(
                        day
                    )}`;
                    hasError = true;
                    return;
                ...
                if (!timeRegex.test(openTime) || !timeRegex.test(closeTime)) {
                    errorMessage = `Format waktu tidak valid untuk ${this.getDayName(
                        day
                    )}`;
                    hasError = true;
                    return;
                ...
                if (openTime >= closeTime) {
                    errorMessage = `Jam tutup harus lebih besar dari jam buka untuk ${this.getDayName(
                        day
                    )}`;
                    hasError = true;
                    return;
                ...
                if (schedule.hasBreakSchedule) {
                    schedule.breakSchedules.forEach((breakSchedule, index) => {
                        ...
                        if (!breakOpen || !breakClose) {
                            errorMessage = `Harap lengkapi jam istirahat untuk ${this.getDayName(
                                day
                            )}`;
                            hasError = true;
                            return;
                        ...
                        if (
                            !timeRegex.test(breakOpen) ||
                            !timeRegex.test(breakClose)
                        ) {
                            errorMessage = `Format waktu istirahat tidak valid untuk ${this.getDayName(
                                day
                            )}`;
                            hasError = true;
                            return;
                        ...
                        if (breakOpen >= breakClose) {
                            errorMessage = `Jam selesai istirahat harus lebih besar dari jam mulai untuk ${this.getDayName(
                                day
                            )}`;
                            hasError = true;
                            return;
                        ...
                        if (breakOpen < openTime || breakClose > closeTime) {
                            errorMessage = `Jam istirahat harus berada dalam jam operasional untuk ${this.getDayName(
                                day
                            )}`;
                            hasError = true;
                            return;
        ...
        const enabledDays = days.filter(
            (day) => this.operationalHours[day].enabled
        );
        if (enabledDays.length === 0) {
            this.showAlert(
                "error",
                "Error",
                "Harap pilih minimal satu hari operasional"
            );
            return;
        }
        ...
        console.log("📤 Sending operational hours data:", dataToSend);
            ...
            .then((data) => {
                ...
                if (data.success) {
                    ...
                    this.showAlert(
                        "error",
                        "Error",
                        data.message || "Gagal menyimpan jam operasional"
                    );
                }
            })
            .catch((error) => {
                ...
                this.showAlert(
                    ...
                    "Error",
                    error.message ||
                        "Terjadi kesalahan saat menyimpan jam operasional"
                );
            });
    }
        ...
        element.innerHTML = `
            <div class="service-header">
                <div class="service-info">
                    ...
                    <div class="service-price">Rp ${service.price.toLocaleString(
                        "id-ID"
                    )}</div>
                    <div class="service-capacity">
                        <span class="service-capacity-label">Kapasitas Harian:</span>
                        <span class="service-capacity-value">${
                            service.capacity
                        } slot</span>
                    </div>
                ...
                <div class="service-actions">
                    <button class="btn--edit"
                            ...
                            aria-label="Edit layanan ${this.escapeHtml(
                                service.name
                            )}">
                        Edit
                    </button>
                    <button class="btn--delete"
                            data-service-id="${service.id}"
                            aria-label="Hapus layanan ${this.escapeHtml(
                                service.name
                            )}">
                        Hapus
                    </button>
    ...
    showAddServiceModal() {
        ...
        const capacityInfoBtn = modalElement.querySelector(
            "#serviceCapacityInfoBtn"
        );
    ...
    showEditServiceModal(serviceId) {
        ...
        const serviceDescriptionInput = modal.querySelector(
            "#serviceDescription"
        );
    ...
    handleDeleteConfirm() {
        ...
        if (serviceIdToDelete === null || serviceIdToDelete === undefined) {
            const recentModal = document.querySelector(
                ".modal-overlay[data-recent-delete]"
            );
        ...
        if (serviceIdToDelete !== null && serviceIdToDelete !== undefined) {
            ...
            this.showAlert(
                "error",
                "Error",
                "Tidak ada layanan yang dipilih untuk dihapus"
            );
        }
    ...
    handleServiceSubmit() {
        ...
        if (!capacity || capacity < 1 || capacity > 50) {
            this.showAlert(
                "error",
                "Error",
                "Harap masukkan kapasitas antara 1-50"
            );
            capacityInput.focus();
            return;
        }
        ...
        if (!description) {
            this.showAlert(
                "error",
                "Error",
                "Harap masukkan deskripsi layanan"
            );
            descriptionInput.focus();
            return;
        }

        // Simpan layanan custom ke database
        const serviceData = {
            ...
            if (serviceIndex !== -1) {
                ...
                this.saveCustomServices()
                    ...
                    .catch((error) => {
                        ...
                        const errorMessage =
                            error.message || "Gagal menyimpan layanan";
                        this.showAlert("error", "Error", errorMessage);
                    });
            }
        } else {
    ...
    getServicesStats() {
        return {
            ...
            totalRevenue: this.services.reduce(
            ...
            averagePrice:
                this.services.length > 0
                    ...
                    : 0,
            totalCapacity: this.services.reduce(
    ...
    getOperationalSummary() {
        ...
        return {
            enabledDays,
            hasBreakSchedule: Object.values(this.operationalHours).some(
                (schedule) => schedule.hasBreakSchedule
            ),
...
document.addEventListener("DOMContentLoaded", () => {
    try {
        ...
        window.getOperationalSummary = () =>
            dashboardManager.getOperationalSummary();
        window.getCurrentState = () => dashboardManager.getCurrentState();

        // Expose notification methods
        window.updateNotification = (type, count) =>
            dashboardManager.updateNotification(type, count);
        window.incrementNotification = (type, amount) =>
            dashboardManager.incrementNotification(type, amount);
        window.decrementNotification = (type, amount) =>
            dashboardManager.decrementNotification(type, amount);
        window.resetNotification = (type) =>
            dashboardManager.resetNotification(type);
        window.getTotalNotifications = () =>
            dashboardManager.getTotalNotifications();

        // Expose floating button methods
        window.resetFloatingButton = () =>
            dashboardManager.resetFloatingButtonPosition();
...
function testServicesWithCapacity() {
    if (window.prismoDashboard) {
        ...
        console.log(
            "📊 Services stats:",
            window.prismoDashboard.getServicesStats()
        );
    }
}
