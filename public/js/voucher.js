...
function attachVoucherCardListeners() {
    ...
    // Attach event listeners to claim buttons in cards
    ...
    claimButtons.forEach(btn => {
        btn.onclick = function(e) {
            ...
            if (voucherId) {
                // Find voucher and set as current
    ...
    console.log('✅ Event listeners attached to', voucherCards.length, 'voucher cards and', claimButtons.length, 'claim buttons');
}
        ...
        <div class="${cardClass}" data-voucher-id="${voucher.id}">
            ...
            <div class="voucher-header" style="background: ${voucherColor};">
                ...
                ${voucher.discountType === 'percentage' && voucher.maxDiscount ? 
                    `<div class="voucher-max-discount" style="font-size: 12px; margin-top: 4px; opacity: 0.9;">Maks. Rp ${formatNumber(voucher.maxDiscount)}</div>` : ''}
            </div>
...
async function claimVoucher() {
    ...
    try {
        const response = await fetch(`/customer/voucher/claim/${currentVoucher.id}`, {
            method: 'POST',
            headers: {
...
function formatNumber(number) {
    // Ensure integer display without decimals
