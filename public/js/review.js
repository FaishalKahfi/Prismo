...
class PrismoReviewManager {
    ...
    getInitialReviews() {
        ...
        const baseReviews = [
            {
                ...
                rating: 5,
                comment:
                    "Pelayanan sangat memuaskan! Mobil jadi bersih mengkilap dan wangi! Teknisinya juga ramah dan profesional. Pasti akan kembali lagi!",
            ...
            },
            {
                ...
                rating: 4,
                comment:
                    "Hasilnya bagus, tapi agak lama menunggu. Mungkin bisa dipercepat prosesnya. Overall puas dengan hasilnya.",
                time: "2 jam yang lalu",
                service: "Premium Steam",
                hasReply: false,
                images: [
            ...
            },
            {
                ...
                rating: 3,
                comment:
                    "Cukup puas dengan pelayanan, tapi ada beberapa area yang kurang bersih. Bagian dalam mobil masih ada debu di sudut-sudutnya.",
            ...
            },
            {
                ...
                rating: 2,
                comment:
                    "Kurang puas dengan pelayanan. Mobil masih ada bekas sabun dan tidak dikeringkan dengan baik. Harus diulang lagi.",
            ...
            },
            {
                ...
                rating: 1,
                comment:
                    "Pelayanan sangat buruk! Mobil saya malah ada baret baru setelah dicuci. Sangat kecewa!",
            ...
            },
            {
                ...
                rating: 5,
                comment:
                    "Excellent service! My car has never been this clean. The team is very professional and thorough.",
        ...
        for (let i = 7; i <= 1248; i++) {
            additionalReviews.push({
                ...
                comment: `Review otomatis ${i}. Pelayanan ${
                    Math.random() > 0.3 ? "sangat memuaskan" : "cukup baik"
                }.`,
    ...
    init() {
        ...
        try {
            ...
            console.error(
                "❌ Failed to initialize PRISMO Review Manager:",
                error
            );
        }
    }
    ...
    setupPaginationEvents() {
        document.addEventListener("click", (event) => {
            ...
            if (
                pageBtn &&
                !pageBtn.classList.contains("pagination__page--active")
            ) {
                event.preventDefault();
    ...
    updatePageHeaderStats() {
        const stats = this.getReviewStats();
        const pendingReplies = this.reviews.filter(
    ...
    createReviewElement(review) {
        ...
        if (review.images && review.images.length > 0) {
            const imagesHTML = this.generateImageGrid(
                review.images,
                review.id,
        ...
        images.forEach((image, index) => {
            imagesHTML += `
                <div class="review-image" data-image-index="${index}" data-review-id="${reviewId}">
                    <img src="${image}" alt="Gambar review ${
                index + 1
            } dari ${reviewerName}" loading="lazy">
                </div>
            `;
        });
    ...
    showReplyModal(review) {
        ...
        form.addEventListener("submit", (event) => {
            event.preventDefault();
            console.log(
                "📤 FORM SUBMIT - Current review ID:",
                this.currentReplyReviewId
            );
            this.handleReplySubmit();
        });
    ...
    handleReplySubmit() {
        ...
        console.log("✅ SENDING REPLY TO BACKEND...");
        ...
        fetch(`/api/reviews/${reviewId}/reply`, {
            ...
            .then((data) => {
                ...
                this.reviews[reviewIndex].hasResponse = true;
                this.reviews[reviewIndex].response = data.mitra_reply;
                this.reviews[reviewIndex].mitraResponse = data.mitra_reply;
                this.reviews[reviewIndex].mitra_reply = data.mitra_reply;
                this.reviews[reviewIndex].replied_at = data.replied_at;
                this.reviews[reviewIndex].hasReply = true;
                this.reviews[reviewIndex].reply = {
                    text: data.mitra_reply,
                    time: "Baru saja",
                };

                this.closeCurrentModal();
                this.displayReviews(); // Refresh the display
                this.updateRatingSummary(); // Update rating summary
                this.showSuccessAlert("Respon berhasil dikirim!");

                // Update notification count
                this.notifications.review = Math.max(
                    0,
                    this.notifications.review - 1
                );
                this.updateAllBadges();

                this.currentReplyReviewId = null; // Reset
                console.log(
                    "🎉 REPLY SUCCESSFULLY SAVED TO DATABASE:",
                    reviewId
                );
            })
            .catch((error) => {
    ...
    showEditReplyModal(review) {
        ...
        const modal = template.content.cloneNode(true);
        const originalReview = modal.querySelector(
            "#editOriginalReviewContent"
        );
    ...
    handleDeleteReply(reviewId) {
        ...
        fetch(`/api/reviews/${reviewId}/reply`, {
            ...
            .then((data) => {
                ...
                this.reviews[reviewIndex].hasResponse = false;
                this.reviews[reviewIndex].reply = null;
                this.reviews[reviewIndex].response = null;
                this.reviews[reviewIndex].mitraResponse = null;
                this.reviews[reviewIndex].mitra_reply = null;
                this.reviews[reviewIndex].replied_at = null;
...
document.addEventListener("DOMContentLoaded", () => {
    console.log(
        "🚀 DOM Content Loaded - Initializing PRISMO Review Manager..."
    );
...
function demoAddReview() {
    if (window.prismoReview) {
        const newReview = {
            ...
            rating: 3,
            comment:
                "Ini adalah review demo yang ditambahkan melalui console. Pelayanan cukup memuaskan!",
