...
// Handle signin form - Use standard form submission for session-based auth
if (signinForm) {
    // Let form submit naturally to server for proper session handling
    // Server-side AuthController will handle redirect based on role
    signinForm.addEventListener("submit", function (e) {
        ...
        // Let form submit naturally - no preventDefault, no API call
    });
}
...
function adjustCustomerTypeToggle() {
    if (window.innerWidth <= 768) {
        ...
        if (signUpForm && customerTypeToggle) {
            ...
            const existingToggle = signUpForm.querySelector(
...
// Modal Syarat & Ketentuan (check if elements exist first)
if (showTermsBtn) {
...
forgotPasswordForm.addEventListener("submit", async (e) => {
    ...
    const submitBtn = forgotPasswordForm.querySelector(".main-btn");
    showLoading(submitBtn);
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const passwordInput = document.querySelector(
        '.sign-up input[name="password"]'
    );
    const confirmPasswordInput = document.querySelector(
        '.sign-up input[name="confirmPassword"]'
    );
    ...
    if (googleSignupBtn && termsCheckbox) {
        googleSignupBtn.addEventListener("click", function () {
            if (!termsCheckbox.checked) {
                showNotification(
                    "Harap centang Syarat & Ketentuan terlebih dahulu",
                    "error"
                );
                // Highlight checkbox
            ...
            window.location.href =
                "/auth/google?action=register&role=" + selectedRole;
        });
    }
});
