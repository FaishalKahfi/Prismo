...
class MitraProfileController extends Controller
{
    public function showForm()
    {
        ...
        // Jika sudah approved, redirect ke dashboard mitra
        if ($user->approval_status === 'approved') {
            return redirect('/dashboard-mitra');
        }

        // Jika status pending dan sudah ada profile, redirect ke halaman pending
        if ($user->approval_status === 'pending' && $user->profile_completed && $user->mitraProfile) {
            return redirect()->route('mitra.form.pending');
        }

        // Jika status rejected, biarkan user mengisi ulang form
        // Hapus profil lama jika ada dan status rejected
        if ($user->approval_status === 'rejected' && $user->mitraProfile) {
    ...
    public function submitForm(Request $request)
    {
        ...
        try {
            ...
            foreach ($request->facilityPhotos as $index => $base64Image) {
                ...
                if (strpos($base64Image, 'data:image/png') !== false) {
                    $imageType = 'png';
                } elseif (strpos($base64Image, 'data:image/jpeg') !== false) {
                    $imageType = 'jpeg';
                } elseif (strpos($base64Image, 'data:image/jpg') !== false) {
                    $imageType = 'jpg';
                }
                ...
                $imageName = 'facility_' . $user->id . '_' . $index . '_' . time() . '.' . $imageType;
            ...
            // Upload KTP photo
            $ktpType = 'jpg'; // default
            if (strpos($request->ktpPhoto, 'data:image/png') !== false) {
                $ktpType = 'png';
            } elseif (strpos($request->ktpPhoto, 'data:image/jpeg') !== false) {
                $ktpType = 'jpeg';
            }
            ...
            $ktpPhotoName = 'ktp_' . $user->id . '_' . time() . '.' . $ktpType;
            Storage::disk('public')->put('mitra/documents/' . $ktpPhotoName, base64_decode($ktpPhoto));

            // Upload QRIS photo
            $qrisType = 'jpg'; // default
            if (strpos($request->qrisPhoto, 'data:image/png') !== false) {
                $qrisType = 'png';
            } elseif (strpos($request->qrisPhoto, 'data:image/jpeg') !== false) {
                $qrisType = 'jpeg';
            }
            ...
            $qrisPhotoName = 'qris_' . $user->id . '_' . time() . '.' . $qrisType;
            ...
            MitraProfile::updateOrCreate(
                ...
                [
                    ...
                    // Set default empty values for operational hours and service packages
            ...
            // Notify admins about new mitra registration
            NotificationService::newMitraRegistration($user);
