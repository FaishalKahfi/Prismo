...
class DetailMitraController extends Controller
{
    public function show($id)
    {
        ...
        $allReviews = Review::where('mitra_id', $mitra->id)->get();
        ...
        if (!empty($customServices)) {
            ...
            // Only show default packages if prices are set (not null or 0)
            $services = [];

            if (!empty($profile->basic_price) && $profile->basic_price > 0) {
                $services[] = [
                    'id' => 1,
                    'name' => 'Basic Steam',
                    'description' => 'Cuci eksterior mobil dengan sabun khusus',
                    'price' => $profile->basic_price
                ];
            }

            if (!empty($profile->premium_price) && $profile->premium_price > 0) {
                $services[] = [
                    'id' => 2,
                    'name' => 'Premium Steam',
                    'description' => 'Pembersihan interior lengkap',
                    'price' => $profile->premium_price
                ];
            }

            if (!empty($profile->complete_price) && $profile->complete_price > 0) {
                $services[] = [
                    'id' => 3,
                    'name' => 'Complete Steam',
                    'description' => 'Paket lengkap cuci exterior + interior',
                    'price' => $profile->complete_price
                ];
            }
        }
