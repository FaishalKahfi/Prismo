...
class NotificationService
{
    ...
    {
        // Only set booking_id if type is for booking
        $data = [
            'user_id' => $userId,
            'type' => $type,
            'title' => $title,
        ...
        if (strpos($type, 'booking_') === 0 && $relatedId !== null) {
            $data['booking_id'] = $relatedId;
        }
        // For voucher or other types, booking_id will be null
    ...
    /**
     * Notify admin when new customer registers
     */
    public static function newCustomerRegistration($customer)
    {
        self::notifyAdmins(
            'customer_new',
            'Customer Baru',
            "Customer baru '{$customer->name}' telah mendaftar di sistem"
        );
    }
